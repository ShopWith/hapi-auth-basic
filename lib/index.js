...
internals.implementation = function (server, options) {
    ...
    const REALM = settings.realm || 'Basic';
    ...
    const scheme = {
        authenticate: async function (request, h) {
            ...
            if (!authorization) {
                throw Boom.unauthorized(null, REALM, settings.unauthorizedAttributes);
            }

            const parts = authorization.split(/\s+/);

            if (parts[0].toLowerCase() !== REALM) {
                throw Boom.unauthorized(null, REALM, settings.unauthorizedAttributes);
            }

            if (parts.length !== 2) {
                throw Boom.badRequest('Bad HTTP authentication header format', REALM);
            }

            const credentialsPart = Buffer.from(parts[1], 'base64').toString();
            const sep = credentialsPart.indexOf(':');
            if (sep === -1) {
                throw Boom.badRequest('Bad header internal syntax', REALM);
            }
            ...
            if (!username &&
                ...
                throw Boom.unauthorized('HTTP authentication header missing username', REALM, settings.unauthorizedAttributes);
            }
            ...
            if (!isValid) {
                return h.unauthenticated(Boom.unauthorized('Bad username or password', REALM, settings.unauthorizedAttributes), credentials ? { credentials } : null);
            }
